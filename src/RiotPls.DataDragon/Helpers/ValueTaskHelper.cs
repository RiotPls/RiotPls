...
namespace RiotPls.DataDragon.Helpers
{
    internal static class ValueTaskHelper
    {
        ...
        public static ValueTask Create<TState>(TState state, bool isSyncPathAvailable,
            Action<TState> syncResult, Func<TState, Task> asyncResult)
        {
            if (!isSyncPathAvailable)
        ...
        public static ValueTask Create<TState>(TState state, Func<TState, bool> isSyncPathAvailable,
            Action<TState> syncResult, Func<TState, Task> asyncResult)
        {
        ...
        public static ValueTask<T> Create<TState, T>(TState state, bool isSyncPathAvailable,
            Func<TState, T> syncResult, Func<TState, Task<T>> asyncResult)
            => isSyncPathAvailable
                ? new ValueTask<T>(syncResult(state))
                : new ValueTask<T>(asyncResult(state));

        public static ValueTask<T> Create<TState, T>(TState state, Func<TState, bool> isSyncPathAvailable,
            Func<TState, T> syncResult, Func<TState, Task<T>> asyncResult)
